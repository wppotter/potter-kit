<?php
/**
 * Potter Advanced Headers Bar Post Meta Box
 *
 * @package   Potter Pro
 */
 ...
 * Meta Boxes setup
 */
class Potter_Target_Rules_Fields {
	...
	public function __construct() {
		define( 'PK_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'PK_TARGET_RULE_VER', '1.0.0' );
		define( 'PK_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );

		add_action( 'admin_action_edit', array( $this, 'initialize_options' ) );
		add_action( 'wp_ajax_potter_get_posts_by_query', array( $this, 'potter_get_posts_by_query' ) );
	}
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'potter_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'potter-kit' ),
			'special-search' => __( 'Search Page', 'potter-kit' ),
			'special-blog'   => __( 'Blog / Posts Page', 'potter-kit' ),
			'special-front'  => __( 'Front Page', 'potter-kit' ),
			'special-date'   => __( 'Date Archive', 'potter-kit' ),
			'special-author' => __( 'Author Archive', 'potter-kit' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'potter-kit' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'potter-kit' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'potter-kit' ),
					'basic-singulars' => __( 'All Singulars', 'potter-kit' ),
					'basic-archives'  => __( 'All Archives', 'potter-kit' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'potter-kit' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'potter-kit' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxanomies, etc.', 'potter-kit' ),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'potter_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'potter-kit' ),
				'value' => array(
					'all'        => __( 'All', 'potter-kit' ),
					'logged-in'  => __( 'Logged In', 'potter-kit' ),
					'logged-out' => __( 'Logged Out', 'potter-kit' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'potter-kit' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.5.0
		 */
		return apply_filters( 'potter_user_roles_list', $selection_options );
	}
	 ...
	 * @since  1.0.0
	 */
	function potter_get_posts_by_query() {
	 ...
	 * This is only run from potter_get_posts_by_query()
	 *
	...
	public function admin_styles() {
		wp_enqueue_script( 'potter-select2', PK_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), PK_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( PK_TARGET_RULE_DIR . 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$ast_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'potter-select2-lang',
					PK_TARGET_RULE_URI . 'i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'potter-select2',
					),
		...
		wp_register_script(
			'potter-target-rule',
			PK_TARGET_RULE_URI . 'target-rule.js',
			array(
				'jquery',
				'potter-select2',
			),
		...
		wp_enqueue_script( 'potter-target-rule' );

		wp_register_script(
			'potter-user-role',
			PK_TARGET_RULE_URI . 'user-role.js',
			array(
				'jquery',
			),
			PK_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'potter-user-role' );

		wp_register_style( 'potter-select2', PK_TARGET_RULE_URI . 'select2.css', '', PK_TARGET_RULE_VER );
		wp_enqueue_style( 'potter-select2' );
		wp_register_style( 'potter-target-rule', PK_TARGET_RULE_URI . 'target-rule.css', '', PK_TARGET_RULE_VER );
		wp_enqueue_style( 'potter-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'potter-kit' ),
			'please_delete' => __( 'Please delete', 'potter-kit' ),
			'more_char'     => __( 'or more characters', 'potter-kit' ),
			'character'     => __( 'character', 'potter-kit' ),
			'loading'       => __( 'Loading more results…', 'potter-kit' ),
			'only_select'   => __( 'You can only select', 'potter-kit' ),
			'item'          => __( 'item', 'potter-kit' ),
			'char_s'        => __( 's', 'potter-kit' ),
			'no_result'     => __( 'No results found', 'potter-kit' ),
			'searching'     => __( 'Searching…', 'potter-kit' ),
			'not_loader'    => __( 'The results could not be loaded.', 'potter-kit' ),
			'search'        => __( 'Search pages / post / categories', 'potter-kit' ),
		);
		wp_localize_script( 'potter-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'potter-kit' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-potter-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="potter-target-rule-condition pkt-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '<select name="' . esc_attr( $input_name ) . '[rule][{{data.id}}]" class="target_rule-condition form-control pkt-input">';
		$output .= '<option value="">' . __( 'Select', 'potter-kit' ) . '</option>';
		...
		$output .= '</div> <!-- potter-target-rule-condition -->';

		/* Specific page selection */
		$output .= '<div class="target_rule-specific-page-wrap" style="display:none">';
		$output .= '<select name="' . esc_attr( $input_name ) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control pkt-input " multiple="multiple">';
		$output .= '</select>';
		$output .= '</div>';
		...
		/* Wrapper Start */
		$output .= '<div class="pkt-target-rule-wrapper pkt-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
		// $output .= '<input type="hidden" class="form-control pkt-input pkt-target_rule-input" name="' . esc_attr( $input_name ) . '" value=' . $value . ' />';
		$output .= '<div class="pkt-target-rule-selector-wrapper pkt-target-rule-' . $rule_type . '-on">';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'potter-kit' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'potter-kit' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'potter-kit' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="potter-target-rule-condition pkt-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . $index . ']" class="target_rule-condition form-control pkt-input">';
			$output .= '<option value="">' . __( 'Select', 'potter-kit' ) . '</option>';
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'potter-kit' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'potter_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-potter-user-role-condition">';
			$output     .= '<div class="potter-user-role-condition pkt-user-role-{{data.id}}" data-rule="{{data.id}}" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[{{data.id}}]" class="user_role-condition form-control pkt-input">';
					$output .= '<option value="">' . __( 'Select', 'potter-kit' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- potter-user-role-condition -->';
		$output             .= '</script>';
		...
		$output         .= '<div class="pkt-user-role-wrapper pkt-user-role-display-on-wrap" data-type="display">';
			$output     .= '<div class="pkt-user-role-selector-wrapper pkt-user-role-display-on">';
				$output .= '<div class="user_role-builder-wrap">';
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="potter-user-role-condition pkt-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[' . $index . ']" class="user_role-condition form-control pkt-input">';
					$output .= '<option value="">' . __( 'Select', 'potter-kit' ) . '</option>';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'potter_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'potter-kit' ), $rule_set_titles );
 ...
 */
Potter_Target_Rules_Fields::get_instance();
