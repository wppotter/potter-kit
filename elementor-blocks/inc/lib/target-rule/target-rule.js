;(function ( $, window, undefined ) {
			...
			ajax: {
			    ...
			    data: function (params) {
			      	return {
				        ...
				        page: params.page,
				        action: 'potter_get_posts_by_query'
			    	};
				},
	...
	var update_target_rule_input = function(wrapper) {
		var rule_input 		= wrapper.find('.pkt-target_rule-input');
		var old_value = rule_input.val();
		var new_value = [];

		wrapper.find('.potter-target-rule-condition').each(function(i) {
	...
	var update_close_button = function(wrapper) {
		...
		type 		= wrapper.closest('.pkt-target-rule-wrapper').attr('data-type');
		rules 		= wrapper.find('.potter-target-rule-condition');
		show_close	= false;
		...
		rules.each(function() {
			if ( show_close ) {
				jQuery(this).find('.target_rule-condition-delete').removeClass('pkt-hidden');
			}else{
				jQuery(this).find('.target_rule-condition-delete').addClass('pkt-hidden');
			}
		});
	};

	var update_exclusion_button = function( force_show, force_hide ) {
		var display_on = $('.pkt-target-rule-display-on-wrap');
		var exclude_on = $('.pkt-target-rule-exclude-on-wrap');

		var exclude_field_wrap = exclude_on.closest('tr');
		var add_exclude_block  = display_on.find('.target_rule-add-exclusion-rule');
		var exclude_conditions = exclude_on.find('.potter-target-rule-condition');

		if ( true == force_hide ) {
			exclude_field_wrap.addClass( 'pkt-hidden' );
			add_exclude_block.removeClass( 'pkt-hidden' );
		}else if( true == force_show ){
			exclude_field_wrap.removeClass( 'pkt-hidden' );
			add_exclude_block.addClass( 'pkt-hidden' );
		}else{
	...
	$(document).ready(function($) {
		...
		jQuery( '.potter-target-rule-condition' ).each( function() {
			var $this 			= $( this ),
		...
		jQuery('.pkt-target-rule-selector-wrapper').each(function() {
		...
		jQuery( document ).on( 'change', '.potter-target-rule-condition select.target_rule-condition' , function( e ) {

			var $this 		= jQuery(this),
				this_val 	= $this.val(),
				field_wrap 	= $this.closest('.pkt-target-rule-wrapper');

			if( 'specifics' == this_val ) {
				$this.closest( '.potter-target-rule-condition' ).next( '.target_rule-specific-page-wrap' ).slideDown( 300 );
			} else {
				$this.closest( '.potter-target-rule-condition' ).next( '.target_rule-specific-page-wrap' ).slideUp( 300 );
			}

			update_target_rule_input( field_wrap );
		} );

		jQuery( '.pkt-target-rule-selector-wrapper' ).on( 'change', '.target-rule-select2', function(e) {
			var $this 		= jQuery( this ),
				field_wrap 	= $this.closest('.pkt-target-rule-wrapper');
		...
		jQuery( '.pkt-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-rule-wrap a', function(e) {
			e.preventDefault();
			e.stopPropagation();
			var $this 	= jQuery( this ),
				...
				rule_wrap = $this.closest('.pkt-target-rule-selector-wrapper').find('.target_rule-builder-wrap'),
				template  = wp.template( 'potter-target-rule-' + type + '-condition' ),
				field_wrap 		= $this.closest('.pkt-target-rule-wrapper');

			rule_wrap.append( template( { id : new_id, type : type } ) );

			init_target_rule_select2( '.pkt-target-rule-'+type+'-on .target-rule-select2' );
		...
		jQuery( '.pkt-target-rule-selector-wrapper' ).on( 'click', '.target_rule-condition-delete', function(e) {
			var $this 			= jQuery( this ),
				rule_condition 	= $this.closest('.potter-target-rule-condition'),
				field_wrap 		= $this.closest('.pkt-target-rule-wrapper');
				cnt 			= 0,
			...
			if ( 'exclude' == data_type ) {
				...
				if ( 1 === field_wrap.find('.target_rule-condition').length ) {
					...
					$this.parent('.potter-target-rule-condition').next('.target_rule-specific-page-wrap').remove();
			...
			field_wrap.find('.potter-target-rule-condition').each(function(i) {
				...
				condition.removeClass('pkt-target-rule-'+old_rule_id).addClass('pkt-target-rule-'+i);
		...
		jQuery( '.pkt-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-exclusion-rule a', function(e) {
			e.preventDefault();
			e.stopPropagation();
