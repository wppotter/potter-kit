<?php
/**
 * Potter Sites Notices
 *
 * Closing notice on click on `potter-notice-close` class.
 *
 ...
 * `<a href="#" data-repeat-notice-after="<?php echo MONTH_IN_SECONDS; ?>" class="potter-notice-close">`
 * It close the notice for 30 days.
 *
 * @package Potter Sites
 * @since 1.4.0
 */
...
if ( ! class_exists( 'Potter_Notices' ) ) :
	...
	class Potter_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_potter-notice-dismiss', array( $this, 'dismiss_notice' ) );
		...
		public function dismiss_notice() {
			...
			if ( false === wp_verify_nonce( $nonce, 'potter-notices' ) ) {
				wp_send_json_error( _e( 'WordPress Nonce not validated.', 'potter-kit' ) );
			}
		...
		public function enqueue_scripts() {
			wp_register_script( 'potter-notices', self::_get_uri() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				'potter-notices',
				'potterNotices',
				array(
					'_notice_nonce' => wp_create_nonce( 'potter-notices' ),
				)
			);
		}
		...
		public function show_notices() {
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `potter-notices-id-<$array-index>`.
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from Potter_Notices.
				'is_dismissible'             => true,
			);
		...
		public static function markup( $notice = array() ) {
			wp_enqueue_script( 'potter-notices' );

			do_action( 'potter_notice_before_markup' );

			do_action( "potter_notice_before_markup_{$notice['id']}" );
			...
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="notice-container">
					<?php do_action( "potter_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "potter_notice_after_markup_{$notice['id']}" );

			do_action( 'potter_notice_after_markup' );
		}
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'potter-notice', 'notice' );
