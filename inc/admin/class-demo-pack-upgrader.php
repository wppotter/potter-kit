...
class TG_Demo_Pack_Upgrader extends WP_Upgrader {
	...
	public function install_strings() {
		$this->strings['no_package'] = __( 'Install package not available.', 'potter-kit' );
		/* translators: %s: package URL */
		$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'potter-kit' );
		$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'potter-kit' );
		$this->strings['remove_old']          = __( 'Removing the old version of the demo&#8230;', 'potter-kit' );
		$this->strings['remove_old_failed']   = __( 'Could not remove the old demo.', 'potter-kit' );
		$this->strings['installing_package']  = __( 'Installing the demo&#8230;', 'potter-kit' );
		$this->strings['no_files']            = __( 'The demo contains no files.', 'potter-kit' );
		$this->strings['process_failed']      = __( 'Demo install failed.', 'potter-kit' );
		$this->strings['process_success']     = __( 'Demo installed successfully.', 'potter-kit' );
	}
	...
	public function check_package( $source ) {
		...
		if ( ! file_exists( $working_directory . 'screenshot.jpg' ) ) {
			return new WP_Error( 'incompatible_archive_no_demos', $this->strings['incompatible_archive'], __( 'No valid demos were found.', 'potter-kit' ) );
		}
