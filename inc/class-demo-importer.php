...
class PK_Demo_Sites {
	...
	public function __construct() {
		...
		// Help Tabs.
		if ( apply_filters( 'potter_kit_importer_enable_admin_help_tab', true ) ) {
		...
		add_action( 'wppotter_ajax_demo_imported', array( $this, 'update_nav_menu_items' ) );
		add_action( 'wppotter_ajax_demo_imported', array( $this, 'update_elementor_data' ), 10, 2 );
		add_action( 'wppotter_ajax_demo_imported', array( $this, 'update_siteorigin_data' ), 10, 2 );

		// Update widget and customizer demo import settings data.
		add_filter( 'wppotter_widget_demo_import_settings', array( $this, 'update_widget_data' ), 10, 4 );
		add_filter( 'wppotter_customizer_demo_import_settings', array( $this, 'update_customizer_data' ), 10, 2 );
	}
	...
	private function get_demo_packages() {
		$packages = get_transient( 'potter_kit_importer_packages' );
		$template = strtolower( str_replace( '-pro', '', get_option( 'template' ) ) );

		if ( false === $packages || ( isset( $packages->slug ) && $template !== $packages->slug ) ) {
			$raw_packages = wp_safe_remote_get( "https://raw.githubusercontent.com/wppotter/potter-kit-demos/master/configs/{$template}.json" );

			if ( ! is_wp_error( $raw_packages ) ) {
				$packages = json_decode( wp_remote_retrieve_body( $raw_packages ) );

				if ( $packages ) {
					set_transient( 'potter_kit_importer_packages', $packages, WEEK_IN_SECONDS );
				}
			}
		}

		return apply_filters( 'potter_kit_importer_packages_' . $template, $packages );
	}
	...
	private function get_import_file_path( $filename ) {
		return trailingslashit( WPPK_DEMO_DIR . '/dummy-data' ) . sanitize_file_name( $filename );
	}
	...
	public function enqueue_scripts() {
		...
		wp_register_style( 'jquery-confirm', $assets_path . 'css/jquery-confirm/jquery-confirm.css', array(), WPPK_VERSION );
		wp_register_style( 'wp-pk', $assets_path . 'css/demo-importer.css', array( 'jquery-confirm' ), WPPK_VERSION );



		// Add RTL support for admin styles.
		wp_style_add_data( 'wp-pk', 'rtl', 'replace' );
		...
		if ( 'potter-kit_page_demo-importer' === $screen_id ) {
			...
			wp_localize_script(
				...
				array(
					'l10n' => array(
						'importing'             => __( 'Importing...', 'potter-kit' ),
						'demoImportingLabel'    => _x( 'Importing %s...', 'demo', 'potter-kit' ), // no ellipsis
						'importingMsg'          => __( 'Importing... please wait.', 'potter-kit' ),
						'importedMsg'           => __( 'Import completed successfully.', 'potter-kit' ),
						'importFailedShort'     => __( 'Import Failed!', 'potter-kit' ),
						'importFailed'          => __( 'Import failed: %s', 'potter-kit' ),
						'demoImportedLabel'     => _x( '%s imported!', 'demo', 'potter-kit' ),
						'demoImportFailedLabel' => _x( '%s import failed', 'demo', 'potter-kit' ),
						'livePreview'           => __( 'Live Preview', 'potter-kit' ),
						'livePreviewLabel'      => _x( 'Live Preview %s', 'demo', 'potter-kit' ),
						'imported'              => __( 'Imported!', 'potter-kit' ),
						'statusTextLink'        => '<a href="https://wppotter.com/potter-kit/docs/" target="_blank">' . __( 'Try this solution!', 'potter-kit' ) . '</a>',
					),
				)
			);
			wp_localize_script(
				...
				array(
					...
					'settings' => array(
						...
						'suggestURI'    => apply_filters( 'potter_kit_importer_suggest_new', 'https://pottertheme.com/support/' ),
						'confirmReset'  => __( 'It is strongly recommended that you backup your database before proceeding. Are you sure you wish to run the reset wizard now?', 'potter-kit' ),
						'confirmImport' => sprintf(

							__( 'Importing demo data will ensure that your site will look similar as theme demo. It makes you easy to modify the content instead of creating them from scratch. Also, consider before importing the demo: %1$s %2$s %3$s %4$s %5$s %6$s', 'potter-kit' ),

							'<ol><li class="warning">' . __( 'Importing the demo on the site if you have already added the content is highly discouraged.', 'potter-kit' ) . '</li>',

							'<li>' . __( 'You need to import demo on fresh WordPress install to exactly replicate the theme demo.', 'potter-kit' ) . '</li>',

							'<li>' . __( 'It will install the required plugins as well as activate them for installing the required theme demo within your site.', 'potter-kit' ) . '</li>',

							'<li>' . __( 'Copyright images will get replaced with other placeholder images.', 'potter-kit' ) . '</li>',

							'<li>' . __( 'None of the posts, pages, attachments or any other data already existing in your site will be deleted or modified.', 'potter-kit' ) . '</li>',

							'<li>' . __( 'It will take some time to import the theme demo.', 'potter-kit' ) . '</li></ol>'

							)
					),
					'l10n'     => array(
						'search'              => __( 'Search Demos', 'potter-kit' ),
						'searchPlaceholder'   => __( 'Search demos...', 'potter-kit' ), // placeholder (no ellipsis)
						/* translators: %s: support forums URL */
						'error'               => sprintf( __( 'An unexpected error occurred. Something may be wrong with Potter Kit demo server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.', 'potter-kit' ), 'https://wordpress.org/support/plugin/potter-kit' ),
						'tryAgain'            => __( 'Try Again', 'potter-kit' ),
						'suggestNew'          => __( 'Please suggest us!', 'potter-kit' ),
						'demosFound'          => __( 'Number of Demos found: %d', 'potter-kit' ),
						'noDemosFound'        => __( 'No demos found. Try a different search.', 'potter-kit' ),
						'collapseSidebar'     => __( 'Collapse Sidebar', 'potter-kit' ),
						'expandSidebar'       => __( 'Expand Sidebar', 'potter-kit' ),
						/* translators: accessibility text */
						'selectFeatureFilter' => __( 'Select one or more Demo features to filter by', 'potter-kit' ),
						'confirmMsg'          => __( 'Confirm!', 'potter-kit' ),
					),
				)
			);
		}
		...
		if ( isset( $current_screen->id ) && apply_filters( 'potter_kit_importer_display_admin_footer_text', in_array( $current_screen->id, array( 'potter-kit_page_demo-importer' ) ) ) ) {
			...
			if ( ! get_option( 'potter_kit_importer_admin_footer_text_rated' ) ) {
				$footer_text = sprintf(
					...
					__( 'If you like %1$s please leave us a %2$s rating. A huge thanks in advance!', 'potter-kit' ),
					sprintf( '<strong>%s</strong>', esc_html__( 'Potter Kit', 'potter-kit' ) ),
					'<a href="https://wordpress.org/support/plugin/potter-kit/reviews?rate=5#new-post" target="_blank" class="potter-kit-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'potter-kit' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
			} else {
				$footer_text = __( 'Thank you for using Potter Kit.', 'potter-kit' );
			}
		}
	...
	public function hide_reset_notice() {
		if ( isset( $_GET['potter-kit-hide-notice'] ) && isset( $_GET['_potter_kit_importer_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_potter_kit_importer_notice_nonce'], 'potter_kit_importer_hide_notice_nonce' ) ) {
			...
			if ( ! empty( $hide_notice ) && 'reset_notice' == $hide_notice ) {
				update_option( 'potter_kit_importer_reset_notice', 1 );
			}
		}
	}
	...
	public function reset_wizard_actions() {
		...
		if ( isset( $_GET['potter_kit_importer_reset_nonce'], $_GET['do_reset_wordpress'] ) ) {
			if ( ! wp_verify_nonce( wp_unslash( $_GET['potter_kit_importer_reset_nonce'] ), 'potter_kit_importer_reset' ) ) { // WPCS: input var ok, sanitization ok.
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'everest-forms' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'You don&#8217;t have permission to do this.', 'potter-kit' ) );
			}
			...
			$footer_rated = get_option( 'potter_kit_importer_admin_footer_text_rated' );
			...
			// Update footer text.
			if ( $footer_rated ) {
				update_option( 'potter_kit_importer_admin_footer_text_rated', $footer_rated );
			}
			...
			$required_plugins = (array) apply_filters( 'potter_kit_importer_' . $template . '_required_plugins', array() );
			if ( is_array( $required_plugins ) ) {
				if ( ! in_array( WPPK_PLUGIN_BASENAME, $required_plugins ) ) {
					$required_plugins = array_merge( $required_plugins, array( WPPK_PLUGIN_BASENAME ) );
				}
	...
	public function ajax_query_demos( $return = true ) {
		 ...
		 */
		$prepared_demos = (array) apply_filters( 'potter_kit_importer_pre_prepare_demos_for_js', array(), $available_packages, $demo_activated_id );
		...
		if ( isset( $available_packages->demos ) ) {
			foreach ( $available_packages->demos as $package_slug => $package_data ) {
				...
				$screenshot_url = "https://raw.githubusercontent.com/wppotter/potter-kit-demos/master/resources/{$available_packages->slug}/{$package_slug}/screenshot.jpg";
		 ...
		 */
		$prepared_demos = apply_filters( 'potter_kit_importer_prepare_demos_for_js', $prepared_demos );
	...
	public function ajax_import_demo() {
		...
		if ( empty( $_POST['slug'] ) ) {
			wp_send_json_error(
				array(
					...
					'errorMessage' => __( 'No demo specified.', 'potter-kit' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'import' ) ) {
			$status['errorMessage'] = __( 'Sorry, you are not allowed to import content.', 'potter-kit' );
		...
		do_action( 'wppotter_ajax_before_demo_import' );
		...
		if ( ! empty( $demo_data ) ) {
			...
			update_option( 'potter_kit_importer_activated_id', $slug );
			do_action( 'wppotter_ajax_demo_imported', $slug, $demo_data );
		}
	...
	public function ajax_footer_text_rated() {
		...
		update_option( 'potter_kit_importer_admin_footer_text_rated', 1 );
		wp_die();
	}
	...
	public function import_dummy_xml( $demo_id, $demo_data, $status ) {
		...
		do_action( 'wppotter_ajax_before_dummy_xml_import', $demo_data, $demo_id );
		...
		if ( is_file( $import_file ) ) {
			...
			do_action( 'wppotter_ajax_dummy_xml_imported', $demo_data, $demo_id );

			flush_rewrite_rules();
		} else {
			$status['errorMessage'] = __( 'The XML file dummy content is missing.', 'potter-kit' );
			wp_send_json_error( $status );
		}
	...
	public function import_customizer_data( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The DAT file customizer data is missing.', 'potter-kit' );
			wp_send_json_error( $status );
		}
	...
	public function import_widget_settings( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The WIE file widget content is missing.', 'potter-kit' );
			wp_send_json_error( $status );
		}
				...
				if ( ! empty( $menu_items ) ) {
					foreach ( $menu_items as $menu_item ) {
						if ( isset( $menu_item->url ) && isset( $menu_item->db_id ) && 'custom' == $menu_item->type ) {
							...
							if ( isset( $menu_parts['path'] ) && isset( $menu_parts['host'] ) && apply_filters( 'potter_kit_importer_nav_menu_item_url_hosts', in_array( $menu_parts['host'], array( 'demo.wppotter.com' ) ) ) ) {
